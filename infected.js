...
var infected = function() {
    var settings = {             // the default settings
            ...
            sickDensity: 0.002,  // the ratio of the world that is sick
            healthyDensity: 0.1, // the ratio of the world that is healthy
    ...
    function init(userSettings) {
        ...
        //populate obstacles
        /* 3 boxes */
        state.obstacles.push({x: Math.floor(state.width * 0.1), y: Math.floor(state.height * 0.1),
                w: Math.floor(state.width * 0.1), h: Math.floor(state.height * 0.6)});
        state.obstacles.push({x: Math.floor(state.width * 0.3), y: Math.floor(state.height * 0.5),
                w: Math.floor(state.width * 0.4), h: Math.floor(state.height * 0.3)});
        state.obstacles.push({x: Math.floor(state.width * 0.8), y: Math.floor(state.height * 0.2),
                w: Math.floor(state.width * 0.1), h: Math.floor(state.height * 0.5)});
        /* fort 
        state.obstacles.push({x: Math.floor(state.width * 0.4), y: Math.floor(state.height * 0.2),
                w: Math.floor(state.height * 0.2), h: 5});
        state.obstacles.push({x: Math.floor(state.width * 0.5), y: Math.floor(state.height * 0.2),
                w: Math.floor(state.height * 0.3), h: 5});
        state.obstacles.push({x: Math.floor(state.width * 0.4), y: Math.floor(state.height * 0.2),
                w: 5, h: Math.floor(state.height * 0.6)});
        state.obstacles.push({x: Math.floor(state.width * 0.6), y: Math.floor(state.height * 0.2),
                w: 5, h: Math.floor(state.height * 0.6)});
        state.obstacles.push({x: Math.floor(state.width * 0.4), y: Math.floor(state.height * 0.8)-1,
                w: Math.floor(state.height * 0.5), h: 5});
        */
        ...
        */
        ...
        openSpaces = state.width * state.height;
        ...
        //populate sick
        for (i = 0; i<openSpaces*settings.sickDensity; i++) {
            var s = randomPos();
        ...
        //populate healthy
        for (i = 0; i<openSpaces*settings.healthyDensity; i++) {
            h = randomPos();
            //keep trying until we aren't inside an obstacle
    ...
    function moveSick(posMap) {
        ...
        while(i--) {
            ...
            sick.oldX = sick.x;
            sick.oldY = sick.y;
            if (Math.random() < 0.1) {  // chance of sick moving is 10%
    ...
    function findOpenSpaceAround(actor, posMap) {
        ...
        for (x = 0; x<xdirections.length; x++) {
            for (y = 0; y<ydirections.length; y++) {
